...
# Test
python DeepPATH_code/02_testing/xClasses/nc_imagenet_eval.py --checkpoint_dir=$CUR_CHECKPOINT --eval_dir=$TEST_OUTPUT --data_dir=$DATA_DIR  --batch_size 200  --run_once --ImageSet_basename=$BASENAME --ClassNumber $NbClasses --mode='0_softmax'  --TVmode='test'
...
## Code commented below - usually used on manually labelled images for performance assessment
...
#"black,darkred,red,orange,gold,yellow,darkolivegreen,green,cyan,royalblue,blueviolet,fuchsia"
#else
...
#python DeepPATH_code/03_postprocessing/0i_Sensitivity_Specificity.py --threshold='0.32,0.10,0.49,0.06' --labelFile $LABEL_FILE --PatientID $PatientID --files_stats $OUTFILENAMEUNIQ --outputPath=$TEST_OUTPUT
